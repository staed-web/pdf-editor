...
export function MobileToolsBar({ onOpenPages }: { onOpenPages?: () => void }) {
  ...
  return (
    <div
      className="flex shrink-0 items-stretch gap-1 border-t border-[var(--border)] bg-[var(--card)]/95 px-2 py-2 backdrop-blur-xl md:hidden"
    ...
    >
      <button
        ...
        className="touch-target flex h-12 w-12 shrink-0 flex-col items-center justify-center gap-0.5 rounded-2xl border border-[var(--border)] bg-[var(--background)] text-[9px] font-medium text-[var(--muted)]"
      >
        ...
        {TOOLS.map((t) => {
          ...
          return (
            <button
              ...
              className={cn(
                "touch-target flex h-12 min-w-[3.25rem] shrink-0 flex-col items-center justify-center gap-0.5 rounded-2xl px-2 text-[9px] font-medium transition-colors",
                active
