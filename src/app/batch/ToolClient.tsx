...
export default function BatchPage() {
  ...
  const processOne = async (
    ...
    switch (op) {
      case "compress": {
        const out = await ops.compressPdf(buf, "medium");
      ...
      case "pdf-to-jpg": {
        const pages = await ops.renderPdfPages(buf, {
          format: "jpeg",
          scale: 2,
          quality: 0.9,
        });
      ...
      case "unlock": {
        const bytes = await ops.unlockPdf(buf, password);
        return [{ name: suggestedName(file.name, "unlocked"), data: bytes }];
      }
      case "protect": {
        const bytes = await ops.protectPdf(buf, password);
        return [{ name: suggestedName(file.name, "protected"), data: bytes }];
      }
      case "watermark": {
        const bytes = await ops.addWatermark(buf, {
      ...
      case "rotate": {
        const bytes = await ops.rotatePages(buf, 90);
