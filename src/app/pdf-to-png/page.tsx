...
export default function Page() {
  ...
  const run = async () => {
    ...
    try {
      const pages = await renderPdfPages(await file.arrayBuffer(), { format: "png", scale, quality: 0.92 });
      setProgress(100);
  ...
  return (
    <MarketingShell>
      <ToolShell tool={tool} options={
        ...
        {parts && (
          <ResultBar
            fileName={parts.length===1?parts[0].name:`pages-png.zip`}
            size={parts.reduce((a,b)=>a+b.data.byteLength,0)}
            meta={`${parts.length} image(s)`}
            onDownload={()=>{
              if (parts.length===1) downloadBytes(parts[0].data, parts[0].name, "image/png");
              else void downloadZip(parts, `pages-png.zip`);
            }}
          />
        )}
      </ToolShell>
