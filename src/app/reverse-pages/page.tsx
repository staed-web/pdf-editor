...
import { reversePages } from "@/lib/pdf/extra-ops";

import { downloadBytes, isPdfFile } from "@/lib/download";

const tool = getTool("reverse-pages")!;
...
export default function Page() {
  ...
  const run = async () => {
    ...
    setBusy(true);
    try {
      setResult(await reversePages(await file.arrayBuffer()));
      toast.success("Done");
    } catch (e) {
      console.error(e);
  ...
  return (
    <MarketingShell>
      <ToolShell tool={tool} options={
        ...
        {result && <ResultBar fileName="reversed.pdf" size={result.byteLength}  onDownload={()=>downloadBytes(result,"reversed.pdf")} />}
      </ToolShell>
    </MarketingShell>
  );
}
